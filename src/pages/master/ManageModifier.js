...
export default function ManageModifier() {
  ...
  const confirmDelete = async () => {
    if (brandToDelete) {
         try {
      const response = await axiosInstance.delete(`/modifier_delete/${brandToDelete}`);
      fetchModifiers();
  ...
  return (
    ...
    <PageLayout>     
      ...
      <Modal show={showModal} onHide={handleCloseModal}>
        ...
        <Modal.Footer>
          <Button variant="success" style={{backgroundColor:"#0F6973"}} onClick={handleCloseModal}>
            Close
          </Button>
          <Button style={{backgroundColor:"#EC917D" ,border:"none"}} onClick={handleSubmit}>
            Create
          </Button>
        </Modal.Footer>
      </Modal>
