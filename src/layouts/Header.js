...
export default function Header() {
  ...
  const headerClass = drawer ? "header-fixed" : "header-full-width"

  return (
    <Section as="header" className={`mc-header ${headerClass}`}>
      {/* <Logo
        src={data?.logo.src}
        alt={data?.logo.alt}
        name={data?.logo.name}
      ...
      <Box className="mc-header-group">
        {/* <Box className="mc-header-left"> */}
          ...
          <Button
            ...
            className={`mc-header-icon toggle ${drawer ? 'sidebar-open' : 'sidebar-closed'}`}
